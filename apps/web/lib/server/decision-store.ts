...
function safePage(value: number | undefined) { return Number.isInteger(value) && Number(value) > 0 ? Number(value) : 1; }
function safePageSize(value: number | undefined) { return Number.isInteger(value) ? Math.max(1, Math.min(100, Number(value))) : 25; }
...
function decisionIndex(record: StoredDecision) {
  ...
  const searchText = [
    receipt.receiptId, receipt.decisionId, receipt.action.requestId, receipt.action.type,
    receipt.action.tool, receipt.action.operation, receipt.action.targetType,
    receipt.action.targetId, receipt.action.environment, receipt.actor.id, receipt.actor.name,
    receipt.actor.framework, receipt.decisionSummary,
...
function matchesQuery(record: StoredDecision, query: DecisionQuery) {
  ...
  if (query.tool && record.receipt.action.tool !== query.tool) return false;
...
function pageRecords(records: StoredDecision[], query: DecisionQuery): DecisionPage {
  ...
  return {
    decisions: filtered.slice(start, start + pageSize),
    page: normalizedPage,
    pageSize,
    total,
    totalPages,
...
export function createMemoryDecisionStore(): DecisionStore {
  return {
    ...
    async query(workspaceId, query) { return pageRecords([...memoryDecisions.values()].filter((item) => item.workspaceId === workspaceId), query); },
    async lineage(workspaceId, requestId) {
      return [...memoryDecisions.values()].filter((item) => item.workspaceId === workspaceId && item.receipt.action.requestId === requestId).sort((a, b) => a.createdAt.localeCompare(b.createdAt));
    },
...
type DecisionRow = {
  id: string; workspace_id: string; project_id?: string | null; environment_id?: string | null;
  source: StoredDecision["source"]; receipt: DecisionReceipt; created_at: string;
  review_state?: DecisionReviewState | null; review_case_id?: string | null; parent_receipt_id?: string | null;
};
  ...
  return {
    async save(record) {
      ...
      const response = await fetchImpl(`${baseUrl}/rest/v1/vetolayer_decisions`, {
        ...
        body: JSON.stringify({
          id: storageId(record.workspaceId, record.id), workspace_id: record.workspaceId,
          project_id: record.projectId ?? null, environment_id: record.environmentId ?? null,
          source: record.source, receipt: record.receipt, created_at: record.createdAt,
          receipt_id: record.receipt.receiptId, decision_id: record.receipt.decisionId,
          request_id: record.receipt.action.requestId, action_operation: record.receipt.action.operation,
          action_tool: record.receipt.action.tool, actor_id: record.receipt.actor.id,
          actor_name: record.receipt.actor.name ?? null,
          target_id: record.receipt.action.targetId ?? record.receipt.action.targetType,
          outcome: record.receipt.outcome, uses_serv: index.usesServ,
          policy_refs: index.policyRefs, policy_keys: index.policyKeys, search_text: index.searchText,
          review_state: record.reviewState ?? "none", review_case_id: record.reviewCaseId ?? null,
    ...
    async list(workspaceId, limit = 50, scope) {
      ...
      const query = new URLSearchParams({ select, workspace_id: `eq.${workspaceId}`, order: "created_at.desc,id.desc", limit: String(safeLimit) });
      ...
      return (await response.json() as DecisionRow[]).map(mapRow);
    },
    async query(workspaceId, input) {
      ...
      if (input.tool) query.set("action_tool", `eq.${input.tool.replace(/[,*()%_]/g, "").slice(0, 120)}`);
      ...
      if (input.policy) {
        const policy = input.policy.replace(/[{},\"]/g, "").trim().slice(0, 200);
        if (policy) query.set("policy_keys", `cs.{${policy}}`);
      }
      if (input.search?.trim()) {
        const search = input.search.trim().replace(/[,*()%_]/g, " ").replace(/\s+/g, " ").slice(0, 200).toLowerCase();
        if (search) query.set("search_text", `ilike.*${search}*`);
      }
      query.set("order", input.sort === "oldest" ? "created_at.asc,id.asc" : input.sort === "outcome" ? "outcome.asc,created_at.desc" : input.sort === "action" ? "action_operation.asc,created_at.desc" : "created_at.desc,id.desc");
      ...
      const totalToken = response.headers.get("content-range")?.split("/")[1];
      const total = totalToken && totalToken !== "*" ? Number(totalToken) : offset + rows.length + (rows.length === pageSize ? 1 : 0);
      const totalPages = Math.max(1, Math.ceil(total / pageSize));
      return { decisions: rows.map(mapRow), page, pageSize, total, totalPages, hasNext: page < totalPages, hasPrevious: page > 1 };
    },
    async lineage(workspaceId, requestId) {
...
export function getDecisionStore(): { store: DecisionStore; persistence: "supabase" | "memory" } {
  ...
  if (environment.persistenceConfigured && environment.supabaseUrl && environment.supabaseServiceRoleKey) return { store: createSupabaseDecisionStore({ url: environment.supabaseUrl, serviceRoleKey: environment.supabaseServiceRoleKey }), persistence: "supabase" };
