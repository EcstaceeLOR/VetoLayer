...
describe("operational analytics", () => {
  it("derives outcomes, policy friction and production volume while excluding demo receipts", () => {
    const report = buildOperationalAnalytics({
      decisions: [stored(0), stored(1), stored(2), stored(0, { id: "demo", source: "demo" })],
      reviews: [],
      auditEvents: [],
      filters,
    });
  ...
  it("calculates review turnaround, unresolved aging and re-evaluation outcomes from review history", () => {
    const unresolved = review({
      ...
      timeline: [{ id: "created-2", type: "created", summary: "Created", createdAt: "2026-09-24T00:00:00.000Z" }],
      receiptLineage: [{ receiptId: "receipt_review_2", outcome: "REVIEW", createdAt: "2026-09-24T00:00:00.000Z", reason: "initial" }],
    });
  ...
  it("aggregates only safe SERV latency and usage metadata", () => {
    const receipt = {
      ...
      providerTrace: {
        ...
        endpoint: "https://secret-provider.example/api",
  ...
  it("builds Decision Explorer drill-downs with the exact analytics scope", () => {
    const url = buildDecisionExplorerUrl({ ...filters, projectId: "prj_1", environmentId: "env_prod" }, { outcome: "BLOCK", policy: "policy_1" });
    expect(url).toContain("projectId=prj_1");
    expect(url).toContain("environmentId=env_prod");
