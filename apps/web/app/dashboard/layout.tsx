...
import { getAuthenticatedIdentity, getAuthenticatedWorkspace } from "../../lib/server/workspace";
...
export default async function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  if (!workspace) {
    ...
    redirect(identity ? "/onboarding" : "/login?error=session_expired&next=/dashboard");
  }
    ...
    <div className="productShell productShellV2">
      <aside className="sidebar productSidebar" aria-label="Workspace navigation">
        ...
        <WorkspaceProjectContext
          ...
          role={workspace.role}
          workspaces={workspace.availableWorkspaces.map(({ workspace: item, membership }) => ({ workspace: item, role: membership.role }))}
          projects={workspace.availableProjects}
          environments={workspace.availableEnvironments}
        />
        <ProductNavigation />
        ...
        <div className="sideFoot" role="status">
          ...
          <small>{workspace.project.name} · {workspace.environment.name}</small>
        </div>
      </aside>

      <main className="dashboardMain productMain" id="main-content" tabIndex={-1}>
        <header className="dashboardTopbar productTopbar">
          <ProductBreadcrumbs workspace={workspace.workspace.name} project={workspace.project.name} environment={workspace.environment.name} />
          <ProductCommandMenu />
          <div className="topbarActions productTopbarActions">
            ...
            <details className="accountMenu">
              <summary aria-label={`Account menu for ${identityLabel}`}>
                ...
                <span className="accountMenuIdentity"><b>{workspace.displayName ?? workspace.workspace.name}</b><small>{workspace.email ?? workspace.role}</small></span>
              </summary>
