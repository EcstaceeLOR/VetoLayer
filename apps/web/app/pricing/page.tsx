...
export default function PricingPage() {
  ...
  return (
    <main className="pricingPage" id="main-content" tabIndex={-1}>
      <nav className="pricingNav" aria-label="Pricing navigation"><Link href="/" aria-label="VetoLayer home"><VetoLayerLogo size="md" /></Link><div><Link href="/">Product</Link><Link href="/demo">Example</Link>{authConfigured ? <Link href="/login">Sign in</Link> : null}</div></nav>
      ...
      <section className="pricingGrid" aria-label="VetoLayer plans">
        {COMMERCIAL_PLAN_ORDER.map((id) => {
          ...
          return <article className={`pricingCard ${id === "team" ? "featured" : ""}`} key={id}>
            ...
            <div className="pricingAction">{id === "developer" ? <ButtonLink tone="primary" href={authConfigured ? "/onboarding" : "/demo"}>Start on Developer</ButtonLink> : <div><strong>Paid activation is not self-serve yet.</strong><small>{plan.billingNote}</small></div>}</div>
          </article>;
        })}
      </section>
