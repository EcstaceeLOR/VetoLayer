...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "INVALID_JSON", message: "Simulation request must be valid JSON." }, { status: 400 });
  }

  const policy = PolicySchema.safeParse(body.policy);
  if (!policy.success) {
    return NextResponse.json({ error: "INVALID_POLICY", issues: policy.error.issues }, { status: 400 });
  }
  ...
  if (!action.success) {
    return NextResponse.json({ error: "INVALID_ACTION", issues: action.error.issues }, { status: 400 });
  }

  const evidenceInput = body.evidence === undefined ? policyStudioSampleEvidence : body.evidence;
  if (!Array.isArray(evidenceInput)) {
    return NextResponse.json({ error: "INVALID_EVIDENCE", message: "evidence must be an array." }, { status: 400 });
  }
  const evidence = evidenceInput.map((item) => EvidenceSchema.safeParse(item));
  const invalidEvidence = evidence.find((item) => !item.success);
  if (invalidEvidence && !invalidEvidence.success) {
    return NextResponse.json({ error: "INVALID_EVIDENCE", issues: invalidEvidence.error.issues }, { status: 400 });
  }
  ...
  const result = await evaluateAction(
    {
      ...
      environment: {
        ...
        incident: {
          ...
          severity: "critical",
          active: true,
          summary: "Active session-token replay vulnerability; proposed patch directly remediates the exposure.",
        },
      },
...
function neverEvidence(): never {
  throw new Error("Unreachable invalid evidence state.");
}
