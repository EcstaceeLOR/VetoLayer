...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ readiness: getIntegrationReadiness(), connections, persistence, scope: { projectId: auth.workspace.projectId, environmentId: auth.workspace.environmentId } });
  } catch {
...
export async function POST(request: Request) {
  ...
  if (!rate.allowed) {
    ...
    return NextResponse.json({ error: { code: "RATE_LIMITED", message: "Too many integration tests. Try again shortly." } }, { status: 429, headers: { "Retry-After": String(retryAfter) } });
  }
  ...
  try {
    ...
    const connections = await store.list(auth.workspace.workspaceId, { projectId: auth.workspace.projectId, environmentId: auth.workspace.environmentId });
    return NextResponse.json({ result, readiness: getIntegrationReadiness(), connections, persistence, scope: { projectId: auth.workspace.projectId, environmentId: auth.workspace.environmentId } });
  } catch {
    return NextResponse.json({ error: { code: "INTEGRATION_TEST_FAILED", message: "The integration test could not be completed safely." } }, { status: 500 });
  }
}
