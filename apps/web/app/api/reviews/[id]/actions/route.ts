...
export async function POST(
  ...
  if (reviewCase.receipt.outcome !== "REVIEW") {
    return NextResponse.json(
      { error: "NOT_REVIEWABLE", message: "Only REVIEW decisions can enter the human review loop." },
      { status: 409 },
    );
  }
  ...
  if (!parsedReview.success) {
    return NextResponse.json(
      { error: "INVALID_REVIEW", issues: parsedReview.error.issues },
      { status: 400 },
    );
  }

  if (reviewCase.context.kind !== "github") {
    return NextResponse.json({ error: "UNSUPPORTED_REVIEW_CONTEXT" }, { status: 501 });
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "REEVALUATION_FAILED", message: "The action remains in REVIEW; no execution was approved." },
      { status: 503 },
    );
  }
}
