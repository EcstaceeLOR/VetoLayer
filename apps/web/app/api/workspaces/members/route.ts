...
export async function GET() {
  ...
  const [members, invitations] = await Promise.all([
    store.listMembers(auth.workspace.workspaceId),
    store.listInvitations(auth.workspace.workspaceId),
  ]);
...
export async function POST(request: Request) {
  ...
  const role = typeof payload.role === "string" && inviteRoles.has(payload.role as Exclude<WorkspaceRole, "owner">)
    ? payload.role as Exclude<WorkspaceRole, "owner">
    : "member";
  ...
  const invitation = {
    ...
    invitedByUserId: auth.workspace.userId,
